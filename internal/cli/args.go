...
func Parse(args []string, getenv func(string) string, out io.Writer) (Options, error) {
	...
	fs.SetOutput(out)
	fs.IntVar(&o.Port, "port", 0, "port for the web UI (0 takes the first free one from 4700 on, so the browser keeps what it allowed the page)")
	...
	fs.StringVar(&o.Agent, "agent", "claude", "coding agent to drive: claude, or claude-wrapper for a host CLI that starts Claude Code for us (needs --agent-bin)")
	fs.StringVar(&o.AgentBin, "agent-bin", "", "path to the agent executable (default: the agent's usual name); for claude-wrapper, the host command and its subcommand, e.g. \"divar-copilot agent\"")
